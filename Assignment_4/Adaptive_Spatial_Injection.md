...
* **The 10% Floor:** We apply a mathematical floor to the mask (`mask = 0.1 + 0.9 * original_mask`) (this is tuned). If a watermarking method operates globally in the frequency domain (like DCT), completely zeroing out the noise in smooth areas would destroy the payload. The 10% floor ensures a faint signal survives everywhere.
