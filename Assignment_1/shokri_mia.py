...
def extract_features(loader, model, device):
    ...
    with torch.no_grad():
        for _, imgs, labels, memberships in tqdm(loader, desc="Extracting Signals"):
            ...
            # Concatenate 29 features for our Meta-Classifier
...
def evaluate_shokri_attack():
    ...
    # 1. Extract Features from Target Model
    X, y = extract_features(pub_loader, target_model, device)
    
    # 2. Split into Train/Test to evaluate the Attack Model locally
    ...
    # predict_proba returns probabilities for [Class 0, Class 1]. We want Class 1 (Member)
    y_scores = clf.predict_proba(X_test)[:, 1]
    
    # 3. Calculate TPR@5%FPR
